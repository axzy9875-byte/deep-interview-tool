...
export function InterviewSession() {
  ...
  const strict = sessionState.questionMode === 'outline';
  ...
  const handleSubmitAnswer = async () => {
    ...
    try {
      ...
      if (!strict && sessionState.currentQuestionIndex + 1 >= sessionState.questions.length) {
        // 总是生成下一个问题，不受最小问题数限制
  ...
  // 计算进度（最少5个问题为100%）
  const minQuestions = strict ? Math.max(1, totalQuestions) : 5;
        ...
        <Group position="apart" mb="md">
          <div style={{ flex: 1 }}>
            ...
            <Text size="sm" color="dimmed">
              ...
              {strict ? `严格按提纲顺序提问，共 ${totalQuestions} 题。` : isReadyToComplete ? '已达到最少问题数，可以结束访谈或继续提问。' : `至少需要 ${minQuestions} 个问题。`}
