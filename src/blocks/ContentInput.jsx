...
import { OutlineSetup } from './OutlineSetup.jsx';
  ...
  const handleFileUpload = async (files) => {
    ...
    for (const file of files) {
      try {
        ...
        updateContentState({ error: `无法读取 ${file.name}：${error.message}` });
      }
    }
  };
      ...
      <Stack spacing="md">
          ...
          <Group position="apart" align="flex-start">
            <div>
              ...
              <Text color="dimmed" size="sm">
                添加访谈提纲或背景材料，然后选择严格按提纲提问或由 AI 分析生成问题。
              </Text>
            </div>
        ...
        <Card withBorder padding="md"><Stack spacing="sm">
          ...
          {locked && <Alert color="blue"><Stack spacing="xs">
            <Text>已有访谈记录，你仍可以查看提纲原题和完整分析。若要使用新问题开始访谈，点击下方按钮，旧记录会自动备份，材料和分析会保留。</Text>
            <Button variant="light" onClick={() => { try { restartKeepingMaterials(); } catch (error) { updateContentState({ error: error.message }); } }}>备份旧记录，保留材料和分析重新设置访谈</Button>
          </Stack></Alert>}
          <Button variant="subtle" size="xs" onClick={() => {
            const archive = localStorage.getItem('interview_session_archive');
            if (!archive) { updateContentState({ error: '暂无备份记录。' }); return; }
            const url = URL.createObjectURL(new Blob([archive], { type: 'application/json' }));
            const link = document.createElement('a'); link.href = url; link.download = '访谈历史备份.json'; link.click(); setTimeout(() => URL.revokeObjectURL(url), 1000);
          }}>下载已备份的访谈记录</Button>
          {contentState.previewNotice && <Alert color="yellow">{contentState.previewNotice}</Alert>}
        </Stack></Card>
        {strict && <OutlineSetup />}
        {!strict && !contentState.analysisResult && <Alert color="blue">请先点击“开始分析内容”，再调用 AI 生成预览。严格按提纲展示原题无需分析。</Alert>}
        {/* 问题预览 */}
        {!strict && contentState.analysisResult && (
            ...
            <Stack spacing="md">
              <Group position="apart" align="flex-start">
                ...
                <Group spacing="xs">
                  ...
                  >
                    {contentState.previewQuestions.length > 0 ? '重新调用 AI 生成' : '调用 AI 生成预览'}
                  </Button>
                </Group>
              </Group>
              ...
              {contentState.previewQuestions.length > 0 && (
                <Card withBorder padding="sm" style={{ backgroundColor: '#f0f7ff' }}>
                  <Stack spacing="sm">
                    ...
                    {contentState.previewOrigin?.type === 'ai' && <Text size="xs" color="dimmed">来源：{contentState.previewOrigin.model} · {new Date(contentState.previewOrigin.generatedAt).toLocaleString()}</Text>}
