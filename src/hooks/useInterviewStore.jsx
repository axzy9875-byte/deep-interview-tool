import { strictQuestionList, parseOutline, isLegacyFallback } from '../../lib/interviewOutline.js';
...
export const useInterviewStore = create((set, get) => ({
  ...
  restartKeepingMaterials: () => {
    const state = get();
    if (state.sessionState.isGeneratingQuestion || state.contentState.isAnalyzing) throw new Error('请等待当前生成结束后再重新设置');
    const retainedContent = state.contentState;
    if (state.sessionState.questions.length || Object.keys(state.sessionState.answers).length) {
      const archive = JSON.parse(localStorage.getItem('interview_session_archive') || '[]');
      if (!Array.isArray(archive)) throw new Error('历史备份格式异常，未更改当前访谈');
      archive.push({ savedAt: new Date().toISOString(), interviewState: state.interviewState, contentState: retainedContent, sessionState: state.sessionState, resultState: state.resultState });
      // A failed backup must leave the current interview untouched.
      localStorage.setItem('interview_session_archive', JSON.stringify(archive));
    }
    get().resetInterview();
    set({ contentState: { ...retainedContent, usedPreviewQuestions: [], error: null }, interviewState: { ...get().interviewState, currentStep: 'content-input' } });
    get().saveSessionData();
  },

  startOutlineInterview: () => {
    const { contentState, sessionState } = get();
    if (contentState.questionMode !== 'outline' || !contentState.outlineConfirmed) throw new Error('请先核对并确认提纲问题');
    const questions = strictQuestionList(contentState.outlineQuestions);

    // Starting from a reviewed outline is a complete replacement action. If an
    // older interview exists, archive it first and retain the current material,
    // analysis and reviewed outline before creating the new session.
    if (sessionState.questions.length || Object.keys(sessionState.answers || {}).length) {
      get().restartKeepingMaterials();
    }
    set(state => ({
      sessionState: { ...state.sessionState, questionMode: 'outline', questions, answers: {}, currentQuestionIndex: 0, isComplete: false, isGeneratingQuestion: false, streamingQuestion: null, error: null },
      contentState: { ...state.contentState, analysisResult: state.contentState.analysisResult || { summary: '严格按用户确认的访谈提纲进行访谈。', keyTopics: [] } },
      interviewState: { ...state.interviewState, currentStep: 'interviewing' }
    }));
    get().saveSessionData();
  },
  ...
  generateQuestionListPreview: async () => {
    if (get().contentState.questionMode === 'outline') throw new Error('严格模式不生成问题，请使用已确认的提纲');
    const { apiState, contentState } = get();
    ...
    try {
      const client = new AIAPIClient(apiState);
      set(state => ({ contentState: { ...state.contentState, previewQuestions: [], previewOrigin: null, questionFeedback: {}, error: null } }));
      ...
      set((state) => ({
        contentState: {
          ...
          previewOrigin: { type: 'ai', model: apiState.model, generatedAt: new Date().toISOString() }
        }
      }));
      ...
      set(state => ({ contentState: { ...state.contentState, error: `AI 问题预览生成失败：${detail}` } }));
      throw error;
    }
  },
  ...
  generateQuestionStream: async () => {
    if (get().sessionState.isGeneratingQuestion) return null;
    if ((get().sessionState.questions.length ? get().sessionState.questionMode || 'ai' : get().contentState.questionMode) === 'outline') {
      const session = get().sessionState;
      if (!session.questions.length) {
        set(state => ({ sessionState: { ...state.sessionState, error: '请先在内容准备中确认提纲，再开始访谈' } }));
        throw new Error('请先在内容准备中确认提纲，再开始访谈');
      }
  ...
  nextQuestion: () => {
    set(state => {
      const strict = state.sessionState.questionMode === 'outline';
      const index = Math.min(state.sessionState.currentQuestionIndex + 1, state.sessionState.questions.length);
      const finished = strict && index >= state.sessionState.questions.length;
      return { sessionState: { ...state.sessionState, currentQuestionIndex: index, isComplete: finished || state.sessionState.isComplete },
        interviewState: { ...state.interviewState, ...(finished ? { currentStep: 'completed' } : {}) } };
    });
    get().saveSessionData();
  },
  ...
  generateInterviewScriptWithStyle: async (style = 'default') => {
    ...
    set((state) => {
      const interviewScripts = { ...state.resultState.interviewScripts };
      delete interviewScripts[style];
      return {
        resultState: {
          ...state.resultState,
          isGenerating: true,
          ...
          interviewScripts,
          interviewScript: state.resultState.currentStyle === style ? null : state.resultState.interviewScript,
  ...
  loadSessionData: () => {
    try {
      ...
      if (savedData) {
        ...
        if (isLegacyFallback(cleanContentState.previewQuestions)) {
          cleanContentState.legacyPreviewBackup = cleanContentState.previewQuestions;
          cleanContentState.previewQuestions = [];
          cleanContentState.questionFeedback = {};
          cleanContentState.previewOrigin = null;
          cleanContentState.previewNotice = '已识别并停用旧版的固定备用问题。内容分析和访谈记录已保留，请选择直接展示提纲原题，或重新调用 AI 生成。';
        }
        ...
        const expectedQuestions = sessionState?.questions?.length || 0;
        const validInterviewScripts = Object.fromEntries(
          Object.entries(resultState?.interviewScripts || {}).filter(([style, script]) => {
            if (!script?.content?.trim()) return false;
            if (style !== 'qa' || !expectedQuestions) return true;
            const coveredQuestions = script.sourceQuestionCount ||
              (script.content.match(/(?:^|\n)\s*(?:\*\*)?Q[：:]/gi) || []).length;
            return coveredQuestions >= expectedQuestions;
          })
        );
        const cleanResultState = {
          ...resultState,
          ...
          streamingScript: null,
          error: null,
          interviewScripts: validInterviewScripts,
          interviewScript: resultState?.interviewScript?.content?.trim() ? resultState.interviewScript : null
        };
