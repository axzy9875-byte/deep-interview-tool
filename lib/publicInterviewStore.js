...
export class PublicInterviewStore {
  ...
  removeOrphanVisitors(data) {
    const referencedVisitors = new Set(data.sessions.map(session => session.visitorHash).filter(Boolean));
    data.visitors = data.visitors.filter(visitor => referencedVisitors.has(visitor.idHash));
  }
  ...
  async deleteCampaign(id) {
    return this.mutate(data => {
      ...
      const sessionsDeleted = data.sessions.filter(session => session.campaignId === id).length;
      data.campaigns = data.campaigns.filter(item => item.id !== id);
      data.sessions = data.sessions.filter(session => session.campaignId !== id);
      this.removeOrphanVisitors(data);
      return { id, sessionsDeleted };
    });
  }
  ...
  async deleteSession(id) {
    return this.mutate(data => {
      ...
      this.removeOrphanVisitors(data);
      return { id, campaignId: session.campaignId };
    });
  }
}
