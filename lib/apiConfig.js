...
export class APIConfigManager {
  ...
  static saveConfig(config) {
    ...
    localStorage.setItem(CONFIG_STORAGE_KEY + ":" + config.baseUrl, JSON.stringify(encryptedConfig));
  ...
  static loadProviderConfig(baseUrl) {
    try {
      const raw = localStorage.getItem(CONFIG_STORAGE_KEY + ':' + baseUrl);
      if (!raw) return null;
      const config = JSON.parse(raw);
      return { ...config, apiKey: CryptoJS.AES.decrypt(config.apiKey, ENCRYPTION_KEY).toString(CryptoJS.enc.Utf8) };
    } catch { return null; }
  }
  ...
  // 清除配置
  static clearConfig() {
    const keys = Object.keys(localStorage).filter(key => key.startsWith(CONFIG_STORAGE_KEY + ':'));
  ...
  static saveSpeechConfig(config) {
    const stored = {
      ...
      apiKey: config.apiKey ? CryptoJS.AES.encrypt(config.apiKey, ENCRYPTION_KEY).toString() : '',
      baseUrl: config.baseUrl, model: config.model,
      authMode: config.authMode || 'apiKey', appId: config.appId || '',
      resourceId: 'volc.bigasr.auc_turbo',
      createdAt: new Date().toISOString()
    };
    localStorage.setItem(SPEECH_CONFIG_STORAGE_KEY, JSON.stringify(stored));
    return stored;
  }

  static loadSpeechConfig() {
    const defaults = { enabled: false, apiKey: '', baseUrl: 'https://ark.cn-beijing.volces.com/api/v3', model: 'doubao-seed-2-0-lite-260428', authMode: 'apiKey', appId: '', resourceId: 'volc.bigasr.auc_turbo' };
    try {
      const stored = localStorage.getItem(SPEECH_CONFIG_STORAGE_KEY);
      if (!stored) return defaults;
      const config = JSON.parse(stored);
      return { ...defaults, ...config, apiKey: config.apiKey ? CryptoJS.AES.decrypt(config.apiKey, ENCRYPTION_KEY).toString(CryptoJS.enc.Utf8) : '' };
